#pragma once
// Vendored from the Adafruit GFX Library rather than taking that whole library
// as a second dependency alongside Arduino_GFX. The file is pure data -- a
// glyph bitmap blob plus a GFXglyph table -- and the GFXfont/GFXglyph types it
// needs are already provided by Arduino_GFX's own gfxfont.h, so the original
// #include <Adafruit_GFX.h> is deliberately dropped. Include this only AFTER
// <Arduino_GFX_Library.h>.
//
// Licence: the Adafruit GFX fonts are converted from GNU FreeFont, which is
// GPL with the font exception. Worth a look before shipping a closed product

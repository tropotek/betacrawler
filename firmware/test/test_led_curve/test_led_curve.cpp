...
void test_phase_zero_is_off() {
  TEST_ASSERT_EQUAL(0, breathingDuty(0, 1000));
}

void test_phase_quarter_is_half_duty() {
  TEST_ASSERT_EQUAL(50, breathingDuty(250, 1000));
}

void test_phase_half_is_peak() {
  TEST_ASSERT_EQUAL(100, breathingDuty(500, 1000));
}

void test_phase_three_quarter_is_half_duty() {
  TEST_ASSERT_EQUAL(50, breathingDuty(750, 1000));
}

void test_phase_wraps_around_period() {
  TEST_ASSERT_EQUAL(breathingDuty(200, 1000), breathingDuty(1200, 1000));
}

void test_degenerate_period_returns_off() {
  // periodMs/2 == 0: no room for a ramp -- must not divide by zero.
  TEST_ASSERT_EQUAL(0, breathingDuty(0, 1));
}

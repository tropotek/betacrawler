...
parameter at all. Beyond that, `app/web/pages/{modes,controller,device}.html` hand-pick which keys
...
parameter does **not** automatically appear anywhere in the UI any more; it needs a page decision
and a written label, same as any other UI change. `field_help.js` no longer exists. A curated page
must handle a key its own board doesn't publish (`field(key).def === null`) explicitly rather than
assume every named key exists.

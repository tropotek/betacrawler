...
const FIELD_HELP = {
  ...
  'esc0.direction': '"unidirectional" treats the low end of the throttle range as stop (the default, and safe for most ESCs); "bidirectional" treats the middle as stop, either side driving forward or reverse. Get this wrong and arming will not behave as expected.',
  'esc0.mode': '"off" commands nothing; "armed" drives Throttle directly; "input" follows a selected receiver channel. Arming always starts at the safe/neutral position and holds briefly (~2s by default) before honouring a commanded value.',
  'esc0.throttle_us': 'Commanded pulse width in armed mode, 1000–2000 microseconds.',
  'esc0.min_us': "Lowest pulse ESC 0 will ever be commanded, 500–1500 microseconds — stop on a unidirectional ESC, full reverse on a bidirectional one. Calibrate this to ESC 0's real endpoint.",
  'esc0.max_us': "Pulse width commanded at the high end of the throttle range, 1500–2500 microseconds. Calibrate this to ESC 0's real endpoint.",
  'esc0.src': "Which receiver channel drives ESC 0 in input mode (ch1–ch12, matching the RX module's own channel numbering). Only shown while off — change it before arming, not while live.",

  'esc1.direction': '"unidirectional" treats the low end of the throttle range as stop (the default, and safe for most ESCs); "bidirectional" treats the middle as stop, either side driving forward or reverse. Get this wrong and arming will not behave as expected.',
  'esc1.mode': '"off" commands nothing; "armed" drives Throttle directly; "input" follows a selected receiver channel. Arming always starts at the safe/neutral position and holds briefly (~2s by default) before honouring a commanded value.',
  'esc1.throttle_us': 'Commanded pulse width in armed mode, 1000–2000 microseconds.',
  'esc1.min_us': "Lowest pulse ESC 1 will ever be commanded, 500–1500 microseconds — stop on a unidirectional ESC, full reverse on a bidirectional one. Calibrate this to ESC 1's real endpoint.",
  'esc1.max_us': "Pulse width commanded at the high end of the throttle range, 1500–2500 microseconds. Calibrate this to ESC 1's real endpoint.",
  'esc1.src': "Which receiver channel drives ESC 1 in input mode (ch1–ch12, matching the RX module's own channel numbering). Only shown while off — change it before arming, not while live.",
